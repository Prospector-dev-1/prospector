...
const Progress = () => {
  ...
  const fetchProgressData = async () => {
    try {
      ...
      const [uploadsResult, callsResult] = await Promise.all([supabase.from('call_uploads').select('*').eq('user_id', user?.id).eq('status', 'completed').order('created_at', {
        ascending: true
      }), supabase.from('calls').select('*').eq('user_id', user?.id).eq('call_status', 'completed').order('created_at', {
        ascending: true
      })]);
      ...
      const confidenceTrend = [...uploadConfidenceTrend, ...liveCallConfidenceTrend].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      ...
      const objectionTrends = [...uploadObjectionTrends, ...liveCallObjectionTrends].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
      ...
      const avgConfidence = allConfidenceScores.length > 0 ? Math.round(allConfidenceScores.reduce((sum, score) => sum + score, 0) / allConfidenceScores.length) : 0;
      ...
      const avgDifficulty = liveCalls.length > 0 ? Math.round(liveCalls.reduce((sum, call) => sum + call.difficulty_level, 0) / liveCalls.length) : 0;
      ...
      const recentAvg = recentConfidenceScores.length > 0 ? recentConfidenceScores.reduce((sum, score) => sum + score, 0) / recentConfidenceScores.length : 0;
      const olderAvg = olderConfidenceScores.length > 0 ? olderConfidenceScores.reduce((sum, score) => sum + score, 0) / olderConfidenceScores.length : recentAvg;
      const improvement = Math.round(recentAvg - olderAvg);
      const insights = [`You've completed ${liveCalls.length + uploads.length} practice sessions - excellent dedication!`, `Your success rate is ${successRate}% - keep building on this momentum!`, `You're practicing at difficulty level ${avgDifficulty} - challenge yourself to grow!`, "Your confidence scores are trending upward across both live and uploaded calls!", "Great mix of live practice and call analysis - this balanced approach accelerates learning!", "Your objection handling has improved significantly across all categories!", "You're building strong sales skills with consistent practice!"];
      setProgressData({
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <Card>
              ...
              <CardContent>
                {progressData?.confidence_trend.length === 0 ? <div className="h-64 flex items-center justify-center text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                {progressData?.objection_trends.length === 0 ? <div className="h-64 flex items-center justify-center text-muted-foreground">
                    No data yet. Upload calls to see trends!
                  </div> : <ResponsiveContainer width="100%" height={300}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <Card>
              ...
              <CardContent>
                {progressData?.success_rate_trend.length === 0 ? <div className="h-64 flex items-center justify-center text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                {progressData?.difficulty_progression.length === 0 ? <div className="h-64 flex items-center justify-center text-muted-foreground">
                    ...
                    <BarChart data={progressData?.difficulty_progression}>
                      ...
                      <YAxis domain={[0, 100]} />
                      <Tooltip formatter={(value, name, props) => [`${value}%`, `Avg Score (${props.payload.callCount} calls)`]} />
